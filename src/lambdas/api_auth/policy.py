...
class AuthPolicy(object):
    ...
    def _add_method(self, effect, verb, resource, conditions, web_socket_route=None):
        """Adds a method to the internal lists of allowed or denied methods. Each object in
        the internal list contains a resource ARN and a condition statement. The condition
        statement can be null."""

        # web socket allow/deny policy management
        if verb is None and web_socket_route is not None:
            resource_arn = 'arn:aws:execute-api:{region}:{account_id}:{api_id}/{stage}/{web_socket_route}'.format(
                region=self.region,
                account_id=self.aws_account_id,
                api_id=self.rest_api_id,
                stage=self.stage,
                web_socket_route=web_socket_route
            )
        else:
            ...
            resource_arn = 'arn:aws:execute-api:{region}:{account_id}:{api_id}/{stage}/{verb}/{resource}'.format(
                region=self.region,
                account_id=self.aws_account_id,
                api_id=self.rest_api_id,
                stage=self.stage,
    ...
    # def allow_ws_connect_route(self):
    #     """Adds a '*' allow to the policy to authorize access to $connect route of a websocket API"""
    #     self._add_method('Allow', verb=None, resource=None, conditions=[], web_socket_route=WebSocketRoute.CONNECT)
